<title>Pengetahuan - Sistem Pakar Swamedikasi</title>
<?php
...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($connect, "SELECT * FROM basis_pengetahuan ORDER BY kode_pengetahuan");
	...
	if ($_POST[Go]){
			$numrows = mysqli_num_rows(mysqli_query($connect, "SELECT * FROM basis_pengetahuan b,penyakit p where b.kode_penyakit=p.kode_penyakit AND p.nama_penyakit like '%$_POST[keyword]%'"));
			if ($numrows > 0){
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>"; 
	$hasil = mysqli_query($connect, "SELECT * FROM basis_pengetahuan b,penyakit p where b.kode_penyakit=p.kode_penyakit AND p.nama_penyakit like '%$_POST[keyword]%'");
	$no = 1;
	$counter = 1;
    while ($r=mysqli_fetch_array($hasil)){
	if ($counter % 2 == 0) $warna = "dark";
	else $warna = "light";
	$sql = mysqli_query($connect, "SELECT * FROM gejala where kode_gejala = '$r[kode_gejala]'");
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>
		  "; 
	$hasil = mysqli_query($connect, "SELECT * FROM basis_pengetahuan ORDER BY kode_pengetahuan limit $offset,$limit");
	$no = 1;
	$no = 1 + $offset;
	$counter = 1;
    while ($r=mysqli_fetch_array($hasil)){
	...
	$sql = mysqli_query($connect, "SELECT * FROM gejala where kode_gejala = '$r[kode_gejala]'");
	$rgejala=mysqli_fetch_array($sql);
	$sql2 = mysqli_query($connect, "SELECT * FROM penyakit where kode_penyakit = '$r[kode_penyakit]'");
  ...
  case "tambahpengetahuan":
	echo "	<div class='alert alert-success alert-dismissible'>
				...
				<b>Contoh:</b><br>
				Jika kepercayaan <b>(MB)</b> anda terhadap gejala Muntah untuk penyakit Maag adalah <b>0.8 (Hampir Pasti)</b><br>
				Dan ketidakpercayaan <b>(MD)</b> anda terhadap gejala Muntah untuk penyakit Maag adalah <b>0.2 (Hampir Mungkin)</b><br><br>
				<b>Maka:</b> CF(Pakar) = MB – MD (0.8 - 0.2) = <b>0.6</b> <br>
				Dimana nilai kepastian anda terhadap gejala Muntah untuk penyakit Maag adalah <b>0.6 (Kemungkinan Besar)</b>
              </div>
		...
		$hasil4 = mysqli_query($connect, "SELECT * FROM penyakit order by nama_penyakit");
		...
		$hasil4 = mysqli_query($connect, "SELECT * FROM gejala order by nama_gejala");
		while($r4=mysqli_fetch_array($hasil4)){
  ...
  case "editpengetahuan":
    $edit=mysqli_query($connect, "SELECT * FROM basis_pengetahuan WHERE kode_pengetahuan='$_GET[id]'");
