<title>Penyakit - Sistem Pakar Swamedikasi</title>
<?php
...
if (!(isset($_SESSION['username']) && isset($_SESSION['password']))) {
  ...
  switch ($_GET[act]) {
    ...
    default:
      ...
      $tampil = mysqli_query($connect, "SELECT * FROM penyakit ORDER BY kode_penyakit");
      ...
      if ($_POST[Go]) {
        $numrows = mysqli_num_rows(mysqli_query($connect, "SELECT * FROM penyakit where nama_penyakit like '%$_POST[keyword]%'"));
        if ($numrows > 0) {
		  ...
		  <tbody>";
          $hasil = mysqli_query($connect, "SELECT * FROM penyakit where nama_penyakit like '%$_POST[keyword]%'");
          $no = 1;
          $counter = 1;
        ...
        if ($baris > 0) {
		  ...
		  <tbody>
		  ";
          $hasil = mysqli_query($connect, "SELECT * FROM penyakit ORDER BY kode_penyakit limit $offset,$limit");
          $no = 1;
          $no = 1 + $offset;
          $counter = 1;
          while ($r = mysqli_fetch_array($hasil)) {
    ...
    case "editpenyakit":
      $edit = mysqli_query($connect, "SELECT * FROM penyakit WHERE kode_penyakit='$_GET[id]'");
