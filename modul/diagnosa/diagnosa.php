<title>Diagnosa - Sistem Pakar Swamedikasi</title>
<?php
switch ($_GET['act']) {

  default:
    if ($_POST['submit']) {
      // warna dari pilihan user
      $arcolor = array('#ffffff', '#cc66ff', '#019AFF', '#00CBFD', '#00FEFE', '#A4F804', '#FFFC00', '#FDCD01', '#FD9A01', '#FB6700');
      date_default_timezone_set("Asia/Jakarta");
      $inptanggal = date('Y-m-d H:i:s');

      // nilai dari interpretasi user
      ...
      $sqlkondisi = mysqli_query($connect, "SELECT * FROM kondisi order by id+0");
      while ($rkondisi = mysqli_fetch_array($sqlkondisi)) {
        $arkondisitext[$rkondisi['id']] = $rkondisi['kondisi'];
      }

      $sqlpkt = mysqli_query($connect, "SELECT * FROM penyakit order by kode_penyakit+0");
...
// --------------------- START ------------------------
      $sqlpenyakit = mysqli_query($connect, "SELECT * FROM penyakit order by kode_penyakit");
      $arpenyakit = array();
      while ($rpenyakit = mysqli_fetch_array($sqlpenyakit)) {
        $cftotal_temp = 0;
        $cf = 0;
        $sqlgejala = mysqli_query($connect, "SELECT * FROM basis_pengetahuan where kode_penyakit=$rpenyakit[kode_penyakit]");
        $cflama = 0;
      ...
      mysqli_query($connect, "INSERT INTO hasil(
                  tanggal,
                  gejala,
                  penyakit,
                  hasil_id,
...
// --------------------- END -------------------------
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
      ...
      foreach ($argejala as $key => $value) {
        ...
        $ig++;
        $gejala = $key;
        $sql4 = mysqli_query($connect, "SELECT * FROM gejala where kode_gejala = '$key'");
	 ...
	 <div class='alert alert-success alert-dismissible'>
                ...
                Silahkan memilih gejala sesuai dengan keluhan anda, anda dapat memilih kepastian keluhan dari 'pasti tidak' sampai 'pasti ya', setelah itu tekan tombol proses (<i class='fa fa-search-plus'></i>)  di bawah untuk melihat hasil.
              </div>
      ...
      $sql3 = mysqli_query($connect, "SELECT * FROM gejala order by kode_gejala");
      $i = 0;
