<title>Post - Sistem Pakar Swamedikasi</title>
...
switch ($_GET[act]) {
    ...
    default:
        ...
        $tampil = mysqli_query($connect, "SELECT * FROM post ORDER BY kode_post");
        ...
        if ($_POST[Go]) {
            $numrows = mysqli_num_rows(mysqli_query($connect, "SELECT * FROM post where nama_post like '%$_POST[keyword]%'"));
            if ($numrows > 0) {
		  ...
		  <tbody>";
                $hasil = mysqli_query($connect, "SELECT * FROM post where nama_post like '%$_POST[keyword]%'");
                $no = 1;
                $counter = 1;
		  ...
		  <tbody>
		  ";
                $hasil = mysqli_query($connect, "SELECT * FROM post ORDER BY kode_post limit $offset,$limit");
                $no = 1;
                $no = 1 + $offset;
                $counter = 1;
                while ($r = mysqli_fetch_array($hasil)) {
    ...
    case "editpost":
        $edit = mysqli_query($connect, "SELECT * FROM post WHERE kode_post='$_GET[id]'");
