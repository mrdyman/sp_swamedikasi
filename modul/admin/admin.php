<title>Admin - Sistem Pakar Swamedikasi</title>
<?php
session_start();
...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($connect, "SELECT * FROM admin ORDER BY username");
	...
	if ($_POST[Go]){
			$numrows = mysqli_num_rows(mysqli_query($connect, "SELECT * FROM admin where username like '%$_POST[keyword]%'"));
			if ($numrows > 0){
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>"; 
	$hasil = mysqli_query($connect, "SELECT * FROM admin where username like '%$_POST[keyword]%'");
	$no = 1;
	$counter = 1;
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>
		  "; 
	$hasil = mysqli_query($connect,"SELECT * FROM admin ORDER BY username limit $offset,$limit");
	$no = 1;
	$no = 1 + $offset;
	$counter = 1;
    while ($r=mysqli_fetch_array($hasil)){
  ...
  case "editadmin":
    $edit=mysqli_query($connect, "SELECT * FROM admin WHERE username='$_GET[id]'");
